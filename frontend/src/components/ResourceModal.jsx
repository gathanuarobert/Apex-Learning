...
import {
  ...
  CheckCircle2, Download, Tag, BookOpen,
} from "lucide-react";
 ...
 *
 * Props:
 *  item           – full resource object (title, description, price, subject, grade, year, thumbnail, is_purchased, owned)
 *  accentClass    – Tailwind color string for the header gradient, e.g. "from-blue-600/20 to-indigo-600/20"
 *  iconBgClass    – e.g. "bg-blue-500/20 text-blue-400"
 *  Icon           – Lucide icon component
 *  payingWallet   – boolean
 *  payingPesapal  – boolean
 *  onPayWallet    – async handler
 *  onPayPesapal   – async handler
 *  onDownload     – called for free / already-purchased items
 *  onClose        – close handler
 *  relatedItems   – optional array of related resource objects
 *  onSelectRelated – called when user clicks a related resource
 */
    ...
    >
      ...
      >
        ...
        <div className={`bg-gradient-to-r ${accentClass} px-6 pt-6 pb-5 border-b border-slate-700/50 flex-shrink-0`}>
          ...
          {meta && (
            <p className="text-slate-400 text-xs font-medium">{meta}</p>
          )}
        </div>
          ...
          {isPurchased && (
            <div className="flex items-center gap-3 p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl">
              ...
              <div>
                ...
                <p className="text-emerald-400/70 text-xs">You can download this resource anytime below.</p>
              </div>
            </div>
          )}
        ...
        <div className="flex-shrink-0 px-6 pb-6 pt-4 border-t border-slate-700/50 space-y-3 bg-[#1e293b]">
          {canDownload ? (
            /* Free or already-purchased: single download button */
            <button
