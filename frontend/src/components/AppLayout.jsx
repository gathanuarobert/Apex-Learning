...
export default function AppLayout({ children }) {
  ...
  // ─── Nav items ────────────────────────────────────────────────────────────
  // Tab-based items navigate to ?tab=xxx so the active highlight survives
  // UserDashboard's state-clearing (it reads ?tab= from the URL now).
  const allNavItems = [
  ...
  const handleNavClick = (item) => {
    ...
    // Use ?tab= query param for tab-based items so the URL itself tracks state
    const dest = item.tab ? `${item.route}?tab=${item.tab}` : item.route;
    navigate(dest);
    setMoreOpen(false);
  };

  // ─── Active state — driven by the URL, never by ephemeral location.state ──
  ...
  const isActive = (item) => {
    ...
    // Dashboard is only active when on /user-dashboard with NO ?tab= param
  ...
  return (
    <div className="relative min-h-screen flex text-slate-200 bg-[#0f172a] font-sans">
      ...
      {/* ════ SIDEBAR ════ */}
      <aside
        className={`hidden md:flex fixed top-0 left-0 bottom-0 z-40 flex-col bg-[#0f172a] border-r border-white/5 transition-all duration-300 ${
      ...
      >
        <header className="md:hidden fixed top-0 left-0 right-0 z-40 flex items-center justify-between px-6 py-4 bg-[#0f172a]/90 backdrop-blur-md border-b border-white/5">
          ...
          >
            <Wallet size={14} /> KSh {walletBalance?.toFixed(0)}
          </button>
        </header>
                        ...
                        >
                          <div className="min-w-0">
                            <p className="text-sm font-bold text-white truncate">
                              {tx.transaction_type === "deposit" ? "Deposit" : "Purchase"}
                            </p>
                          ...
                          <span
                            className={`font-black text-sm whitespace-nowrap ml-4 ${
        ...
        {mobileMainItems.map((item, idx) => (
          <button
            ...
            className={`flex flex-col items-center gap-1 flex-1 py-2 transition-colors ${
              isActive(item) ? "text-blue-400" : "text-slate-500"
            }`}
          >
            <item.icon size={20} />
