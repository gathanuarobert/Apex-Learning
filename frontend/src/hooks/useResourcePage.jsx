 ...
 * @param {string}   downloadPath   – base path segment, e.g. "exams" → resources/exams/:id/download/
 */
export function useResourcePage(fetchFn, resourceType, downloadPath) {
  ...
  const { curricula, grades, subjects, filtered } = useMemo(() => {
    ...
    const subjects  = [...new Set(
      items.filter((i) =>
        (!curriculum || i.curriculum === curriculum) && (!grade || i.grade === grade)
  ...
  // ── Related resources (same subject, excluding current) ──────────────────
