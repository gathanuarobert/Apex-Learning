...
export default function AdminDashboard() {
  ...
  const [editingResource, setEditingResource] = useState(null); // For edit modal
  ...
  // Mobile nav items (condensed for bottom bar)
  ...
  // REPLACE THE buildChartDataFromRealData FUNCTION IN YOUR AdminDashboard.jsx WITH THIS:
...
function buildChartDataFromRealData(users, transactions) {
  ...
  for (let i = 7; i >= 0; i--) {
    ...
    const weekDownloads = weekTransactions.filter(
      (t) => t.transaction_type === "purchase" || t.transaction_type === "download"
    ).length;
  ...
  // Count total users who joined in the 8-week period
  const totalWeeklyUsers = weeks.reduce((sum, w) => sum + w.users, 0);
  console.log("Users who joined in last 8 weeks:", totalWeeklyUsers);
  console.log("Total existing users:", users.length);
  
  // If NO users joined in last 8 weeks but users DO exist,
  // show ALL users in a single "All Time" bar
  if (totalWeeklyUsers === 0 && users.length > 0) {
    const totalRevenue = transactions
      .filter(t => t.transaction_type === "deposit")
      .reduce((sum, t) => sum + parseFloat(t.amount || 0), 0);
    const totalDownloads = transactions
      .filter(t => t.transaction_type === "purchase" || t.transaction_type === "download")
      .length;
  ...
  const fetchDashboardData = async () => {
    try {
      const [usersRes, transactionsRes, resourcesRes, categoriesRes] = await Promise.all([
        ...
        api.get("resources/notes/admin/all/"),
        api.get("resources/news-categories/"),
      ]);

      console.log("Users fetched:", usersRes.data?.length || 0);
      console.log("Transactions fetched:", transactionsRes.data?.length || 0);
  ...
  const deleteUser = async (id) => {
    ...
    try {
      ...
      setUserHistory((prev) => [
        ...prev,
        { id: Date.now(), user: userToDelete.email || userToDelete.username, action: "deleted", timestamp: new Date().toISOString() },
      ]);
    } catch (error) {
  ...
  return (
    <div className="relative min-h-screen flex text-white bg-gray-900 overflow-hidden">
      ...
      {/* ======================== DESKTOP SIDEBAR ======================== */}
      <aside
        className={`hidden md:flex fixed top-0 left-0 h-full bg-gradient-to-b from-gray-800/80 via-gray-900/80 to-gray-800/80 backdrop-blur-md border-r border-gray-700 shadow-lg z-20 flex-col transform transition-all duration-300 ${
                  ...
                  {users.map((u) => (
                    <tr key={u.id} className="border-b border-gray-800 hover:bg-gray-700/30 text-xs md:text-sm">
                      ...
                      <td className="px-2 md:px-4 py-2">
                        <button onClick={() => deleteUser(u.id)} className="text-red-400 hover:text-red-600 text-xs md:text-sm">
                          Delete
                        </button>
                      </td>
                    </tr>
                    ...
                    <tr key={`${resource.type}-${resource.id}`} className="border-b border-gray-800 hover:bg-gray-700/30 text-xs md:text-sm">
                      ...
                      <td className="px-2 md:px-4 py-2">
                        <div className="flex items-center gap-2">
                          <button onClick={() => openEditModal(resource)} className="text-blue-400 hover:text-blue-600" title="Edit">
                            <FaEdit size={14} />
                          </button>
                          <button onClick={() => deleteResource(resource)} className="text-red-400 hover:text-red-600" title="Delete">
                            <FaTrash size={14} />
                          </button>
                        </div>
                      </td>
            ...
            <form onSubmit={handlePostNews} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 md:px-4 py-2 text-white text-sm focus:outline-none focus:border-cyan-500"
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={6}
                  className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 md:px-4 py-2 text-white text-sm focus:outline-none focus:border-cyan-500 resize-none"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    accept="image/*"
                    className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 md:px-4 py-2 text-gray-400 text-sm focus:outline-none focus:border-cyan-500"
                  />
                </div>
              </div>
              <button
                ...
                className="w-full md:w-auto px-6 py-3 bg-cyan-600 hover:bg-cyan-500 rounded-lg font-semibold text-sm disabled:opacity-50 disabled:cursor-not-allowed"
              >
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-xl transition-all active:scale-90 ${
                isActive ? "text-white bg-gray-700/70" : "text-gray-400 hover:text-gray-200"
              }`}
            >
...
function TabButton({ active, onClick, icon, label, isCollapsed }) {
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-3 px-3 py-3 rounded-lg transition ${active ? "bg-cyan-500/20 text-cyan-400 shadow-lg" : "text-gray-300 hover:bg-gray-700/50"} ${
