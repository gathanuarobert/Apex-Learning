...
export default function PastPapersPage() {
  ...
  const { curricula, grades, subjects, items } = useMemo(() => {
    let filtered = papers;
    if (curriculum)
      filtered = filtered.filter((p) => p.curriculum === curriculum);
    if (grade) filtered = filtered.filter((p) => p.grade === grade);
    if (subject) filtered = filtered.filter((p) => p.subject === subject);
    const curricula = [
      ...new Set(papers.map((p) => p.curriculum).filter(Boolean)),
    ].sort();
    const grades = [
      ...new Set(
        papers
          .filter((p) => !curriculum || p.curriculum === curriculum)
          .map((p) => p.grade)
          .filter(Boolean),
      ),
    ].sort();
    const subjects = [
      ...new Set(
        papers
          .filter(
            (p) =>
              (!curriculum || p.curriculum === curriculum) &&
              (!grade || p.grade === grade),
          )
          .map((p) => p.subject)
          .filter(Boolean),
  ...
  const handleDirectDownload = async (item) => {
    try {
      const toast = document.createElement("div");
      toast.className =
        "fixed top-20 right-4 bg-purple-600 text-white px-4 py-3 rounded-lg shadow-lg z-[60] flex items-center gap-2 animate-slide-in";
      toast.innerHTML =
      ...
      const response = await api.get(
        `resources/past-papers/${item.id}/download/`,
        { responseType: "blob" },
      );
  ...
  return (
    <div className="relative min-h-screen text-white bg-gray-900">
      ...
      <div className="flex-1 px-4 py-6 pb-4">
        {loading && (
          <div className="flex flex-col items-center justify-center py-32 gap-3 text-purple-500">
            <Loader2 size={32} className="animate-spin" />
            <p className="text-sm font-medium">Loading past papers…</p>
          </div>
        )}
        {!loading && error && (
        ...
        {!loading && !error && papers.length === 0 && (
          <div className="flex flex-col items-center justify-center py-32 gap-3 text-gray-600">
            <FileArchive size={48} className="opacity-40" />
            <p className="text-sm font-medium">No past papers uploaded yet.</p>
          </div>
        )}
        {!loading && !error && papers.length > 0 && (
          <>
          ...
          >
            <div className="bg-gradient-to-r from-purple-600/30 to-pink-600/20 px-5 py-4 border-b border-gray-800 flex items-start justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5">
                  ...
                  {modal.item.year && `· ${modal.item.year}`}
                </p>
              </div>
              <button
            ...
            <div className="px-5 py-5 flex flex-col gap-3">
              ...
              <button
                onClick={payWallet}
                disabled={paying}
                className="w-full py-3 rounded-xl bg-purple-600 hover:bg-purple-500 font-bold text-sm transition hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50"
              >
