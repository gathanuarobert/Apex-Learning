...
export default function NotesPage() {
  ...
  const { curricula, grades, subjects, items } = useMemo(() => {
    let filtered = notes;
    if (curriculum)
      filtered = filtered.filter((n) => n.curriculum === curriculum);
    if (grade) filtered = filtered.filter((n) => n.grade === grade);
    if (subject) filtered = filtered.filter((n) => n.subject === subject);
    const curricula = [
      ...new Set(notes.map((n) => n.curriculum).filter(Boolean)),
    ].sort();
    const grades = [
      ...new Set(
        notes
          .filter((n) => !curriculum || n.curriculum === curriculum)
          .map((n) => n.grade)
          .filter(Boolean),
      ),
    ].sort();
    const subjects = [
      ...new Set(
        notes
          .filter(
            (n) =>
              (!curriculum || n.curriculum === curriculum) &&
              (!grade || n.grade === grade),
          )
          .map((n) => n.subject)
          .filter(Boolean),
  ...
  const handleDirectDownload = async (item) => {
    try {
      const toast = document.createElement("div");
      toast.className =
        "fixed top-20 right-4 bg-blue-600 text-white px-4 py-3 rounded-lg shadow-lg z-[60] flex items-center gap-2 animate-slide-in";
      toast.innerHTML =
        ...
        >
          ...
          >
            <div className="bg-gradient-to-r from-blue-600/30 to-purple-600/20 px-5 py-4 border-b border-gray-800 flex items-start justify-between">
              <div>
                <p className="text-xs text-gray-400 mb-0.5">Purchase Note</p>
            ...
            <div className="px-5 py-5 flex flex-col gap-3">
              ...
              <button
                onClick={payWallet}
                disabled={paying}
                className="w-full py-3 rounded-xl bg-blue-600 hover:bg-blue-500 font-bold text-sm transition hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50"
              >
